...
import com.shortcuts.app.service.RecorderSessionStopReason
...
class RecorderCleanupViewModel : ViewModel() {
    ...
    override fun onCleared() {
        ...
        if (AutomationRecorder.isRecording.value && context != null) {
            // System Back can remove this route without the recorder header being tapped.
            // Stop the foreground session but PRESERVE captured steps as a reviewable draft
            // (Fix #1). The user gets to decide whether to save or discard when they reopen.
            AutomationRecorder.stopRecording(context, RecorderSessionStopReason.DISCARDED)
        }
        // When not recording, do NOT clear the draft — the user may return to review/save it.
    }
}
...
fun ShortcutsNavigation(startDestination: String = "dashboard") {
    ...
    NavHost(navController = navController, startDestination = startDestination) {
        ...
        composable(RECORDER_ROUTE) { backStackEntry ->
            val recorderCleanup: RecorderCleanupViewModel = viewModel(viewModelStoreOwner = backStackEntry)
            recorderCleanup.bindContext(context)
            RecorderScreen(
                viewModel = automationViewModel,
                onNavigateBack = { navController.popBackStack() }
            )
