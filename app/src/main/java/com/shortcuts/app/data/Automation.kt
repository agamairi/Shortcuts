...
data class Action(
    ...
    /** The package name of the app that contained the target element at record time. */
    ...
    /** Physical display context captured with a coordinate fallback so replay can reject unsafe changes. */
    val recordedDisplayWidth: Int? = null,
    val recordedDisplayHeight: Int? = null,
    val recordedDisplayRotation: Int? = null,
    val recordedDensityDpi: Int? = null,
    ...
    /** When true, replay skips node-selector lookup and goes straight to the recorded coordinate
     *  (still geometry-guarded by coordinateReplayPlan). Default false: match by description, as today. */
    val preferCoordinateReplay: Boolean = false
)
