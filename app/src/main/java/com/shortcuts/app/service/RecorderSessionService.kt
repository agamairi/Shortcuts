...
/** Why a recorder foreground session ended; only an explicit finished recording earns a success notice. */
enum class RecorderSessionStopReason {
    USER_FINISHED,
    ACCESSIBILITY_DISCONNECTED,
    DISCARDED;

    fun shouldShowFinishedNotification(): Boolean = this == USER_FINISHED
}
...
class RecorderSessionService : Service() {
    ...
    override fun onDestroy() {
        serviceScope?.cancel()
        val finalCount = AutomationRecorder.recordedActions.value.size
        stopForeground(STOP_FOREGROUND_REMOVE)
        val stopReason = consumeStopReason()
        if (stopReason?.shouldShowFinishedNotification() == true) {
            RecorderSessionNotifier.showFinishedNotification(this, finalCount)
        }
        super.onDestroy()
    }
    ...
    companion object {
        ...
        @Volatile
        private var pendingStopReason: RecorderSessionStopReason? = null

        private fun consumeStopReason(): RecorderSessionStopReason? {
...
object RecorderSessionNotifier {
    ...
    fun recordingNotification(context: Context, count: Int = 0): android.app.Notification {
        ...
        val contentIntent = PendingIntent.getActivity(
            context,
            0,
            Intent(context, com.shortcuts.app.ui.MainActivity::class.java),
        ...
        val builder = android.app.Notification.Builder(context, CHANNEL_ID)
            ...
            .addAction(
                android.app.Notification.Action.Builder(
                    android.graphics.drawable.Icon.createWithResource(context, android.R.drawable.ic_menu_myplaces),
                    "Mark a tap",
                    PendingIntent.getBroadcast(
                        context,
                        1,
                        Intent(context, MarkTapReceiver::class.java),
    ...
    fun showFinishedNotification(context: Context, count: Int) {
        val manager = notificationManager(context)
        val contentIntent = PendingIntent.getActivity(
            context,
            0,
        ...
        val builder = android.app.Notification.Builder(context, CHANNEL_ID)
            .setContentTitle("Shortcut recorded")
            .setContentText(RecorderNotificationPresenter.formatFinishedContentText(count))
        ...
        manager.notify(1339, builder.build())
    }
