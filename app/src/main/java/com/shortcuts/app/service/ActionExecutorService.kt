...
class ActionExecutorService(
    ...
    private val cancellation: ExecutionCancellation = NoExecutionCancellation
) {
    private var lastSuccessDetail: String? = null

    fun executeActions(actions: List<Action>, shortcutName: String = "Shortcut"): RunResult {
        cancellation.enterExecution()
        return try {
            executeActionsSerially(actions, shortcutName)
        } finally {
            cancellation.leaveExecution()
            // A stopped run may have interrupted the executor while it was in a blocking wait.
            // Do not return that interrupted state to a shared Dispatchers.IO worker.
            Thread.interrupted()
        }
    }

    private fun executeActionsSerially(actions: List<Action>, shortcutName: String): RunResult {
        val results = mutableListOf<StepResult>()
        val details = mutableListOf<String?>()
        actions.forEachIndexed { index, action ->
            if (cancellation.isCancellationRequested) {
                results += cancelledStep()
                details += null
                return cancelledRun(shortcutName, results, details, index + 1, actions.size)
            }

            val result = executeActionSerially(action)
            results += result
            details += lastSuccessDetail

            if (cancellation.isCancellationRequested) {
                // The active step may already have completed, but the user stopped the chain
                // before it could advance. Reporting it as stopped prevents a false success.
                results[results.lastIndex] = cancelledStep()
                return cancelledRun(shortcutName, results, details, index + 1, actions.size)
            }

            if (result !is StepResult.Success && !action.continueOnError) {
                return incompleteRun(shortcutName, results, details, index + 1, actions.size, index + 1)
            }
            ...
            action.delayMillis
                ...
                ?.let { delay ->
                    ...
                    if (action.actionType == ActionType.UI_AUTOMATION && service != null) {
                        service.waitForScreenToSettle(delay)
    ...
    fun executeAction(action: Action): StepResult {
        cancellation.enterExecution()
        return try {
            if (cancellation.isCancellationRequested) cancelledStep() else executeActionSerially(action)
        } finally {
            cancellation.leaveExecution()
            Thread.interrupted()
        }
    ...
    private fun executeActionSerially(action: Action): StepResult {
    ...
    private fun handleWait(action: Action): StepResult {
        ...
        return if (waitForDuration(duration)) StepResult.Success else cancelledStep()
    }
    ...
    private fun handleHttpRequest(action: Action): StepResult {
        ...
        return try {
            ...
            val requestBody = spec.body ?: if (spec.method.permitsRequestBody()) "" else null
            requestBuilder.method(spec.method, requestBody?.toRequestBody())
            ...
            if (cancellation.isCancellationRequested) cancelledStep()
            else failed(FailureReason.NETWORK_ERROR, "The web request couldn't be completed. Check your connection and address.")
        }
    }
    ...
    private fun handleUiAutomation(action: Action): StepResult {
        val uiActionType = action.uiActionType?.trim()?.uppercase()
        if (uiActionType != null && uiActionType !in supportedUiActionTypes) {
            return failed(
                FailureReason.UI_AUTOMATION_FAILED,
                "The recorded screen action \"${action.uiActionType}\" isn't supported by this version of RepeatKit."
            )
        }
        ...
        val service = accessibilityService ?: AutomationAccessibilityService.instance
            ?: return failed(
                ...
                "Turn on RepeatKit Accessibility Service to run this step."
            )

        when (uiActionType) {
            "LONG_PRESS" -> return ReplayUiActionExecutor(service).executeLongPress(action).toStepResult()
            "PRESS_ENTER" -> return ReplayUiActionExecutor(service).executePressEnter(action).toStepResult()
        }

        val hasTarget = !action.targetText.isNullOrBlank() || !action.targetNodeId.isNullOrBlank() || !action.target.isNullOrBlank()
        val isTypeAction = action.uiActionType in setOf("TYPE", "TYPE_TEXT", "TEXT_ENTRY", "INPUT_TEXT", "SET_TEXT")
        if (isTypeAction && !action.textInput.isNullOrBlank() && !hasTarget) {
            val root = service.rootInActiveWindow
            val focusedNode = root?.findFocus(android.view.accessibility.AccessibilityNodeInfo.FOCUS_INPUT)
            if (focusedNode != null) {
                val arguments = android.os.Bundle().apply {
                    putCharSequence(android.view.accessibility.AccessibilityNodeInfo.ACTION_ARGUMENT_SET_TEXT_CHARSEQUENCE, action.textInput)
                }
                val success = focusedNode.performAction(android.view.accessibility.AccessibilityNodeInfo.ACTION_SET_TEXT, arguments)
                focusedNode.recycle()
                root.recycle()
                if (success) return StepResult.Success
            } else {
                root?.recycle()
            }
        }
    ...
    private fun needsPermission(permission: String): StepResult {
        val humanReadable = when (permission) {
            android.Manifest.permission.CAMERA -> "Camera access"
            android.Manifest.permission.BLUETOOTH_CONNECT -> "Nearby devices (Bluetooth) access"
            else -> permission
        }
        return StepResult.NeedsPermission(
            permission = humanReadable,
    ...
    private fun ReplayUiActionResult.toStepResult(): StepResult = when (this) {
        ReplayUiActionResult.Success -> StepResult.Success
        is ReplayUiActionResult.Failure -> failed(FailureReason.UI_AUTOMATION_FAILED, message)
    }

    private fun cancelledStep() = failed(FailureReason.DEVICE_UNAVAILABLE, "The shortcut was stopped.")

    /** Sleeps in small intervals so Stop can interrupt a long configured wait promptly. */
    private fun waitForDuration(durationMillis: Long): Boolean {
        var remaining = durationMillis
        while (remaining > 0 && !cancellation.isCancellationRequested) {
            val interval = minOf(remaining, CANCELLATION_POLL_MILLIS)
            try {
    ...
    private fun incompleteRun(
        shortcutName: String,
        results: MutableList<StepResult>,
        details: MutableList<String?>,
        firstSkippedIndex: Int,
        actionCount: Int,
        failedStepNumber: Int
    ): RunResult {
        repeat(actionCount - firstSkippedIndex) {
            results += StepResult.Skipped("Skipped because step $failedStepNumber did not complete")
            details += null
        }
        return RunResult(shortcutName, results, details)
    }

    private fun cancelledRun(
        shortcutName: String,
        results: MutableList<StepResult>,
        details: MutableList<String?>,
        firstSkippedIndex: Int,
        ...
        repeat(actionCount - firstSkippedIndex) {
            results += StepResult.Skipped("Skipped because the shortcut was stopped")
            details += null
        }
        return RunResult(shortcutName, results, details)
    }
    ...
    companion object {
        ...
        private val supportedUiActionTypes = setOf(
            "CLICK", "TAP", "PRESS", "LONG_PRESS", "PRESS_ENTER",
            "TYPE_TEXT", "SET_TEXT", "TEXT_ENTRY", "TYPE", "INPUT_TEXT",
