...
```
fix: harden accessibility replay and recorder lifecycle

- Request Android's accessibility gesture capability and reject coordinate fallback taps when
  the recorded display context no longer matches.
- Recycle unused accessibility nodes, bound replay traces, and serialize shortcut execution so
  concurrent triggers cannot interfere.
- Make recorder stop reasons explicit, prevent stale tap-mark overlays, and discard a live
  recording cleanly when the user backs out.
- Add regression coverage and update the action/recorder/execution architecture notes.
```
