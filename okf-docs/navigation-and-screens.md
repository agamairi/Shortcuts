...
When a user taps a saved shortcut's card on the Dashboard, the engine first computes its `ShortcutRunSafety` effects.
- If there are no irreversible or outward-facing effects, the shortcut runs immediately via `ShortcutRunner` with the source `ShortcutRunSource.DASHBOARD`.
- If there are irreversible or outward-facing effects, a `DashboardRunConfirmationDialog` is displayed to gain explicit user consent before the run proceeds.
...
- Both `ManualBuilderScreen` and `AiBuilderScreen` now start with zero steps and an empty "No steps yet" state (there are no phantom placeholder steps).
- Both builder screens intercept the system Back button when there are unsaved steps (or an in-progress AI draft). They display a discard-confirmation dialog to prevent silent loss of work. They only pop the `NavBackStackEntry` directly if there are no unsaved changes or the user confirms the discard.
...
- Background tasks like accessibility recording are owned by services (`RecorderSessionService`), ensuring they survive configuration changes and ordinary app backgrounding. An explicit system Back from the recorder route is intentionally a discard: it stops the foreground recording and disarms its overlay before the route is popped, so recording is never left running without a visible in-app control surface.
