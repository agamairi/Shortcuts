...
| `uiActionType` | String? | Recorder/replay sub-type (`TAP`, `TYPE_TEXT`, `SCROLL`, `LONG_PRESS`, `PRESS_ENTER`); legacy aliases may still deserialize | `UI_AUTOMATION` |
...
| `recordedDisplayWidth` | Int? | Physical display width captured with a coordinate fallback | `UI_AUTOMATION` |
| `recordedDisplayHeight` | Int? | Physical display height captured with a coordinate fallback | `UI_AUTOMATION` |
| `recordedDisplayRotation` | Int? | Display rotation captured with a coordinate fallback | `UI_AUTOMATION` |
| `recordedDensityDpi` | Int? | Display density captured with a coordinate fallback | `UI_AUTOMATION` |
