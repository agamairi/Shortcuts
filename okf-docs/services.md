...
## Overview
This document details the background service infrastructure of the **RepeatKit** Android app, specifically focusing on `ModelDownloaderService`, `AutomationAccessibilityService`, and `ActionExecutorService`.
...
1. **Node Traversal & Search**:
   ...
   - If all semantic selectors miss, falls back to a coordinate tap (`screenX`, `screenY`) only when the recorded display size, rotation, and density still match the current display; otherwise it asks for a re-record.
   - Replay waits up to 5 seconds for the target to show up.
2. **Gesture & Input Simulation**:
   - `performClick(node)` or coordinate tap; `accessibility_service_config.xml` requests Android's `canPerformGestures` capability for coordinate dispatch.
...
An explicit recorder Back action is different from temporary backgrounding: it stops the session
as discarded and disarms any tap-mark overlay. `RecorderSessionService` only presents “Shortcut
recorded” for a normal user finish, so a confirmed accessibility disconnect or discarded session
cannot produce a contradictory success notification.
...
`AutomationExecutionService` is a foreground service that guarantees execution survives UI navigation and widget taps. It delegates to `ActionExecutorService`, which acts as the orchestrator between high-level `Action` data objects and lower-level execution services. A fair process-wide execution lock queues concurrent chains, and the service retains its foreground lifetime until every queued start has completed.
